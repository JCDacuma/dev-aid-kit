...
function encodeLine(value: string, urlSafe: boolean): string {
  return Base64.encode(value, urlSafe);
}
...
function processText(
  ...
  if (!perLine) {
    return mode === "encode" ? encodeLine(text, urlSafe) : decodeLine(text);
  }
  return text
    .split("\n")
    .map((line) =>
      mode === "encode" ? encodeLine(line, urlSafe) : decodeLine(line),
    )
    .join("\n");
}
    ...
    <motion.button
      ...
      className={`flex items-center gap-1.5 rounded-md border px-3 py-1.5 font-mono text-xs transition-colors duration-150 ${
        disabled
          ...
          : primary
            ? "border-blue-400/30 bg-blue-400/10 text-blue-400 hover:border-blue-400/50 hover:bg-blue-400/15"
  ...
  return (
    <div
      ...
      className={`relative flex flex-col overflow-hidden rounded-lg border bg-white/2 transition-colors duration-150 ${
        isDragOver ? "border-blue-400/60 bg-blue-400/5" : "border-white/10"
      }`}
    >
      <div className="flex items-center justify-between border-b border-white/10 px-4 py-2.5">
        ...
        <div className="flex items-center gap-3">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.92 }}
            disabled={chars === 0}
            className="flex items-center gap-1 font-mono text-[11px] text-white/40 transition-colors hover:text-blue-400 disabled:cursor-not-allowed disabled:hover:text-white/40"
          >
...
export default function Base64EncoderDecoder() {
  ...
  const handleProcess = useCallback(() => {
    ...
    try {
      const result = processText(inputText, mode, urlSafe, perLine);
  ...
  }, [inputText, mode, urlSafe, perLine]);
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-4 px-5 py-12 sm:px-8 sm:py-16">
        <header className="flex flex-col gap-3">
          <h1 className="flex items-center gap-2 font-mono text-2xl font-semibold tracking-tight text-white sm:text-3xl">
            <span className="flex h-9 w-9 items-center justify-center rounded-md border border-white/15 bg-black/40 text-blue-400">
              <Binary size={18} strokeWidth={1.75} />
            </span>
            Base64 Encoder / Decoder
          </h1>
          <p className="max-w-xl text-sm leading-relaxed text-white/65 sm:text-[15px]">
            Encode or decode Base64 strings entirely in your browser. Drop a
            text file in, or paste directly. Nothing is uploaded anywhere.
          </p>
        </header>
          ...
          <AnimatePresence mode="wait">
            {validation.status === "valid" && (
              <motion.div
                ...
                exit={{ opacity: 0, y: -4 }}
                transition={{ duration: 0.2, ease: EASE }}
                className="flex items-center gap-1.5 font-mono text-xs text-blue-400"
              >
